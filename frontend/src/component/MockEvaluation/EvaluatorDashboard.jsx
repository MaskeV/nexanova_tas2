// frontend/src/component/MockEvaluation/EvaluatorDashboard.jsx
...
const EvaluatorDashboard = () => {
  ...
  useEffect(() => { fetchEvaluations(); }, [statusFilter]);
  ...
  const openEvaluationForm = async (evaluation) => {
    try {
      ...
      const criteria = ev.technology?.evaluationCriteria?.filter(c => c.roundNumber === ev.roundNumber) || [];
      const existingScores = ev.scores || [];

      let initialScores = criteria.map(c => {
        const existing = existingScores.find(s => s.criteriaName === c.criteriaName);
        return { criteriaName: c.criteriaName, maxScore: c.maxScore, score: existing?.score ?? '', description: c.description || '' };
      });

      if (initialScores.length === 0) {
        initialScores.push({ criteriaName: 'Overall Score', maxScore: 100, score: ev.totalScore ?? '', description: '' });
      }

      setScoreData({ scores: initialScores, comments: ev.comments || '' });
  ...
  const handleScoreChange = (index, value) => {
    setScoreData(prev => {
      ...
      updated[index] = { ...updated[index], score: isNaN(parsed) ? '' : Math.min(parsed, max) };
  ...
  const handleSubmit = async (e) => {
    ...
    if (invalid) { toast.warning('Please fill in all scores'); return; }
    try {
      setSubmitting(true);
      const totalScore = scoreData.scores.reduce((sum, s) => sum + parseFloat(s.score), 0);
      await submitEvaluation(selectedEvaluation._id || selectedEvaluation.evaluationId, {
  ...
  const getStatusBadgeClass = (status) => ({
    pending: 'badge-warning', 'in-progress': 'badge-info', completed: 'badge-success', cancelled: 'badge-danger',
  }[status] || 'badge-secondary');

  const totalPossible = scoreData.scores.reduce((sum, s) => sum + s.maxScore, 0);
  const currentTotal = scoreData.scores.reduce((sum, s) => sum + (parseFloat(s.score) || 0), 0);
  ...
  if (loading) {
    return (
      <PageLayout title="🎯 My Evaluations">
        <div className="loading-container"><div className="spinner"></div><p>Loading your evaluations...</p></div>
      </PageLayout>
    );
  }
      ...
      <div className="evaluator-dashboard">
        ...
        <div className="summary-cards">
          {[
            ...
            <div key={s.label} className="summary-card" style={{ borderTop: `3px solid ${s.color}` }}>
              <span className="summary-number" style={{ color: s.color }}>{s.value}</span>
        ...
        {evaluations.length === 0 ? (
          <div className="empty-state"><p>No evaluations assigned to you yet.</p></div>
        ) : (
              ...
              <div key={ev._id || ev.evaluationId} className="evaluation-card">
                <div className="eval-card-header">
                  <div className="eval-participant">
                    ...
                    <span className="eval-email">{ev.participant?.email || ''}</span>
                  </div>
                ...
                {ev.status === 'completed' && ev.totalScore != null && (
                  <div className="eval-score-display">
                    <span>Score: </span><strong>{ev.totalScore}</strong>
                ...
                {ev.status !== 'completed' && ev.status !== 'cancelled' && (
                  <div className="eval-card-actions">
                    <button className="btn btn-primary btn-sm" onClick={() => openEvaluationForm(ev)}>
            ...
            <div className="modal-content modal-large" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <div>
                  ...
                  <p className="modal-subtitle">
                    {selectedEvaluation.participant?.username} — {selectedEvaluation.technology?.name} · Round {selectedEvaluation.roundNumber}
                  </p>
                </div>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="score-progress">
                  <span>Current Total: <strong>{currentTotal}</strong> / {totalPossible}</span>
                  <div className="progress-bar">
                    <div className="progress-fill" style={{ width: `${totalPossible > 0 ? (currentTotal / totalPossible) * 100 : 0}%` }} />
                  </div>
                </div>
                  ...
                  {scoreData.scores.map((item, idx) => (
                    <div key={idx} className="criteria-score-row">
                      <div className="criteria-info">
                        ...
                        {item.description && <span className="criteria-desc">{item.description}</span>}
                      </div>
                      <div className="score-input-wrapper">
                        <input
                          ...
                          placeholder="0" required className="score-input"
                        />
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    onChange={e => setScoreData(prev => ({ ...prev, comments: e.target.value }))}
                    rows={4}
